...
char *pathname_g;
char   *name1[64], *name2[64] ;  // assume at most 64 components in line (for link)
...
void findname(MINODE *pmip, int ino, char *bname, MINODE *mip, int unlink)
{
  ...
  for(int i=0; i<12; i++)
  {
    if(ip->i_block[i] != 0)
    {
      ...
      if(dp->rec_len == BLKSIZE) // deallocate block if only entry
      {
        bdealloc(dev, ip->i_block[i]);
        while(ip->i_block[i + 1])
        {
...
void ch_mod(char *line)
{
  tokenize1(line);
  char *file = name1[2];
  char *permissions = name1[1];
  int mode = 0;
  printf("permissions = %s\n", permissions);
  printf("file = %s\n", file);

  int ino = getino(file, 0);
  MINODE *mip = iget(dev, ino);
  INODE *ip = &(mip->INODE);

  mode = permissions[0]-48 << 6;
	mode |= permissions[1]-48 << 3;
	mode |= permissions[2]-48;
	printf("mode %d\n", mode);  // 4 for r // 2 for w // 1 for x // 0 for -
